...
class festo:
    ...
    def readVacuumPressure(self, id):
        ...
        print("write 8 byte data: ", (", ".join(hex(b) for b in inputData)))
        time.sleep(0.5)  # wait 0.5s
        response = self.ser.read(7)
        print("read 7 byte data:", (", ".join(hex(b) for b in response)))
    ...
    def writePressure(self, id, pressure):
        ...
        inputData = inputData+pressureDec+crc16
    ...
    def writeKp(self, id, kp):
        ...
        self.ser.write(inputData)
        print("write 17 byte data:", (", ".join(hex(b) for b in inputData)))
        time.sleep(0.5)  # wait 0.5s
        response = self.ser.read(8)
        print("read 8 byte data:", (", ".join(hex(b) for b in response)))
        print("CRC checK: ", self.__checkCRC(response[:-2], response[-2:]))
...
if __name__ == "__main__":
    ...
    while (True):
        ...
        if (inputText == 'chamber'):
            chamberPressure = festoObj.readChamberPressure(1)
            print('chamberPressure: %s' % (chamberPressure))
        if (inputText == 'vacuum'):
            vacuumPressure = festoObj.readVacuumPressure(1)
            print('vacuumPressure: %s' % (vacuumPressure))
        if (inputText == 'set'):
            pressure = input("請輸入壓力值:")
            vacuumPressure = festoObj.writePressure(1, int(pressure))
